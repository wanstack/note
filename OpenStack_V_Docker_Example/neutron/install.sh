...
function install() {
    if [[ ${ALL_IN_ONE} != True ]]; then
        if [[ ${CURRENT_NODE_TYPE} =~ "controller" ]];then
            install_modules=(neutron-server neutron-dhcp-agent neutron-l3-agent neutron-metadata-agent neutron-openvswitch-agent dnsmasq)
        else
            install_modules=(neutron-openvswitch-agent)
        fi
    else
        install_modules=(neutron-server neutron-dhcp-agent neutron-l3-agent neutron-metadata-agent neutron-openvswitch-agent dnsmasq)
    fi
