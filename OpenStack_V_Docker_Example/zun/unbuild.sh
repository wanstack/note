...
function build() {
    if [[ ${ALL_IN_ONE} != True ]]; then
        if [[ ${CURRENT_NODE_TYPE} =~ "controller" ]];then
            unbuild_modules=(zun-api zun-wsproxy kuryr)
        else
            unbuild_modules=(zun-cni-daemon zun-compute kuryr)
        fi
    else
        unbuild_modules=(zun-api zun-wsproxy zun-cni-daemon zun-compute kuryr)
    fi
