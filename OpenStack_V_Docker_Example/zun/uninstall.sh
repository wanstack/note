...
function uninstall() {
    if [[ ${ALL_IN_ONE} != True ]]; then
        if [[ ${CURRENT_NODE_TYPE} =~ "controller" ]];then
            uninstall_modules=(zun-api zun-wsproxy kuryr)
        else
            uninstall_modules=(zun-cni-daemon zun-compute kuryr)
        fi
    else
        uninstall_modules=(zun-api zun-wsproxy zun-cni-daemon zun-compute kuryr)
    fi
