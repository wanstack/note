...
function compute_kuryr() {
    ...
    if [[ $count -lt 1 ]];then
        test -d /usr/lib64/python3.6/site-packages/greenlet-1.0.0.dist-info/ || mv /usr/lib64/python3.6/site-packages/greenlet* /tmp/
        tar -zxvf /opt/pip/v/kuryr-libnetwork.tar.gz -C /var/lib/kuryr/
        chown -R kuryr:kuryr /var/lib/kuryr/kuryr-libnetwork
        pip install --no-index --prefix=/usr --find-links=/opt/pip/v flask
...
function create_resource() {
    source /etc/admin.openrc
    user_count=$(openstack user list | grep zun | wc -l)
    if [[ $user_count -lt 1 ]];then
        openstack user create --domain default --password $ZUN_PASSWD zun
        openstack role add --project service --user zun admin
    fi

    service_count=$(openstack service list | grep container | wc -l)
...
function main() {
    if [[ $ALL_IN_ONE =~ True ]];then
        controller_create_kuryr_resource
        compute_kuryr
    else
