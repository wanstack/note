...
function build() {
    if [[ ${ALL_IN_ONE} != True ]]; then
        if [[ ${CURRENT_NODE_TYPE} =~ "controller" ]];then
            unbuild_modules=(nova-api nova-conductor nova-scheduler nova-novncproxy nova-serialproxy)
        else
            unbuild_modules=(nova-compute nova-libvirt)
        fi
    else
        unbuild_modules=(nova-api nova-conductor nova-scheduler nova-novncproxy nova-serialproxy nova-compute nova-libvirt)
    fi
